...
class DamageManager {
    ...
    getPartById(id: number) {
        var found = this.parts.find(element => element.id == id);
    ...
    setHitIndicator(position: Vector3Mp) {
        if (!mp.browsers.exists(overlayBrowser))
            return false;

        var rotation = this.getHitRotationFrom(new mp.Vector3(position.x, position.y, position.z));

        if (rotation > 45 && rotation < 135)
            overlayBrowser.execute("hitTop();");

        if (rotation > 135 && rotation < 225)
            overlayBrowser.execute("hitLeft();");

        if (rotation > 225 && rotation < 315)
            overlayBrowser.execute("hitBottom();");

        if (rotation > 315 && rotation < 360)
            overlayBrowser.execute("hitRight();");
    }
    ...
    setHitmarker() {
        if (!mp.browsers.exists(overlayBrowser))
            ...
            overlayBrowser.execute("showHitmarker();");
    }
    ...
    playHeadshot() {
        ...
        overlayBrowser.execute("headshot();");
    }
