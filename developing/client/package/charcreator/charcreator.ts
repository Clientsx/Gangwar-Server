...
import { Data } from 'charcreator/charcreatordata';
...
function colorForOverlayIdx(index: number): any {
    let color;
...
function updateAppearance(index: number) {
    try {
        ...
        mp.console.logInfo(`${error}`, false, false);
    }
}

function updateHairAndColors() {
    mp.players.local.setComponentVariation(2, Data.hairList[currentGender][hairItem.Index].ID, 0, 2);
...
function fillHairMenu() {
    hairItem = new UIMenuListItem("Hair", "Your character's hair.", new ItemsCollection(Data.hairList[currentGender].map((x: { Name: any; }) => x.Name)));
...
function resetHairAndColorsMenu(refresh: boolean = false) {
    hairItem.Index = 0;
...
creatorMainMenu.ListChange.on((item: NativeUI.UIMenuListItem, listIndex: number) => {
    if (item == genderItem) {
...
creatorMainMenu.ItemSelect.on((item: NativeUI.UIMenuListItem, index: number) => {
    switch (index) {
        case 1:
...
creatorParentsMenu.ItemSelect.on((item: NativeUI.UIMenuListItem, index: number) => {
    switch (item.Text) {
        case "Randomize":
...
creatorParentsMenu.ListChange.on((item: NativeUI.UIMenuListItem, listIndex: number) => {
    updateParents();
});
...
creatorFeaturesMenu.ItemSelect.on((item: NativeUI.UIMenuListItem, index: number) => {
    switch (item.Text) {
        case "Randomize":
            for (let i = 0; i < Data.featureNames.length; i++) {
...
creatorFeaturesMenu.ListChange.on((item: NativeUI.UIMenuListItem, listIndex: number) => {
...
// CREATOR APPEARANCE
let appearanceItems: any = [];
let appearanceOpacityItems: any = [];
let opacities = [];
...
creatorAppearanceMenu.ItemSelect.on((item: NativeUI.UIMenuListItem, index: number) => {
    switch (item.Text) {
        case "Randomize":
            for (let i = 0; i < Data.appearanceNames.length; i++) {
...
creatorAppearanceMenu.ListChange.on((item: NativeUI.UIMenuListItem, listIndex: number) => {
...
// CREATOR HAIR & COLORS
let hairItem: NativeUI.UIMenuListItem;
let hairColorItem: NativeUI.UIMenuListItem;
let hairHighlightItem: NativeUI.UIMenuListItem;
...
let creatorHairMenu = new Menu("Hair & Colors", "", new Point(50, 50));
fillHairMenu();

creatorHairMenu.ItemSelect.on((item: NativeUI.UIMenuListItem, index: number) => {
    switch (item.Text) {
        case "Randomize":
...
creatorHairMenu.ListChange.on((item: NativeUI.UIMenuListItem, listIndex: number) => {
    if (item == hairItem) {
...
mp.events.add("toggleCreator", (active, charData) => {
    if (active) {
        ...
        if (charData) {
            ...
            // hair and colors
            let hair: any = Data.hairList[currentGender].find((h: { ID: any; }) => h.ID == charData.Hair.Hair);
